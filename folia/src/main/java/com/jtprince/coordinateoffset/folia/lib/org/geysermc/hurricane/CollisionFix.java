    ...
    public CollisionFix(Plugin plugin, boolean bambooEnabled, boolean pointedDripstoneEnabled) {
        ...
        if (bambooEnabled) {
            try {
                ...
                bambooBoundingBox = ReflectionAPI.getFieldAccessible(bambooBlockClass, "SHAPE_COLLISION");
                if (bambooBoundingBox != null) {
                    plugin.getLogger().fine("Found bamboo collision hack reflection mapping: SHAPE_COLLISION");
                }
                if (bambooBoundingBox == null) {
                    bambooBoundingBox = ReflectionAPI.getFieldAccessible(bambooBlockClass, "COLLISION_SHAPE");
                    if (bambooBoundingBox != null) {
        ...
        if (pointedDripstoneEnabled) {
            ...
            try {
                ...
                for (Field field : speleothemBlockClass.getDeclaredFields()) {
