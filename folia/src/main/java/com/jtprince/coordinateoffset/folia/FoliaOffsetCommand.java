...
public class FoliaOffsetCommand {
    ...
    public FoliaOffsetCommand(CoordinateOffsetFoliaPlugin plugin, CoordinateOffsetCore core) {
        this.plugin = plugin;
        this.core = core;
    }
    ...
    private int regenerate(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        OffsetRegenerateCommand offsetRegenerateCommand = new OffsetRegenerateCommand(
            toSender(context.getSource().getSender()),
            targets.stream().map(FoliaOffsetPlayer::new).toList()
        );
    ...
    private static OffsetCommandSender toSender(CommandSender sender) {
        return new OffsetCommandSender(sender, sender.getName(), sender instanceof Player ? new FoliaOffsetPlayer((Player) sender) : null);
    }
